...
create(){
  ...
  then
    ...
    aws --profile ${AWS_PROFILE} s3 cp ${TMP_DIR}/${key}.pem s3://${S3_BUCKET_PREFIX}-${CLUSTER_NAME}-config/keypairs/${key}.pem
...
destroy(){
  ...
  then
    ...
    then
      ...
      aws --profile ${AWS_PROFILE} s3 rm s3://${S3_BUCKET_PREFIX}-${CLUSTER_NAME}-config/keypairs/${key}.pem
